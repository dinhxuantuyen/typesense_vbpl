...
**Cách A — docker compose (khuyến nghị, kèm Dashboard quản trị):**
```bash
mkdir -p /opt/legal/import && cd /opt/legal
# tai docker-compose.deploy.yml tu repo ve day, roi:
TYPESENSE_API_KEY='DOI_KEY_MANH' EMBED_API_KEY='sk-xxxx' \
  docker compose -f docker-compose.deploy.yml up -d
# -> MCP:      http://<server>:8000/mcp
# -> Dashboard: http://<server>:8888  (Host=<server>, Port=8108, Protocol=http, API key = TYPESENSE_API_KEY)
```

**Cách B — docker run (chỉ services, không dashboard):**
```bash
...
```
> Dashboard là app tĩnh chạy trong **trình duyệt của bạn** và gọi thẳng Typesense (8108),
> nên cổng 8108 + 8888 chỉ nên mở trong mạng nội bộ/VPN, không public internet.
