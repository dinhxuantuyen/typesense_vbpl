...
| TASK-009 | Dockerfile all-in-one + Phase B import | Image full 378k gặp chuỗi sự cố đã xử lý: (1) WSL Service crash dưới I/O nặng → update WSL 2.7.10 + .wslconfig + build "gentle" (slim + docker cp từng part + commit); (2) layer >10GB vượt giới hạn GHCR → tách 3 layer; (3) thiếu thư mục rỗng (models/, state/snapshot) khi tách part; (4) **braft thiếu snapshot → boot xóa db** → rebuild index + POST /operations/snapshot trước khi dừng; (5) entrypoint fix wait/set-e. Đang rebuild index có snapshot. | In Progress | 2026-07-04 |
